...
if(isset($_POST)) {
	...
	$stmt = $conn->prepare("INSERT INTO candidate_apply(course_id, name, email, mobile) VALUES (?,?, ?, ?)");

	$stmt->bind_param("ssss",$jobid, $name,$email,$number);
    $jobid = mysqli_real_escape_string($conn, $_POST['courseid']);
	...
	if($stmt->execute()) {
		...
		echo "<script>";
		echo "alert('Thank You !! Your Application has Been Submitted.');";
		echo "window.location = 'courses-list.php';"; // redirect with javascript, after page loads
		echo "</script>";
		exit();
	} else {
